...
public class ProvidersRxCacheEncryptedTest {
    ...
    @Test public void _00_Save_Record_On_Disk_In_Order_To_Test_Following_Tests() {
        ...
        providersRxCache.getMocksEncryptedWithDetailResponse(createObservableMocks(SIZE)).subscribe(subscriber);

        subscriber = new TestSubscriber<>();
        providersRxCache.getMocksNotEncryptedWithDetailResponse(createObservableMocks(SIZE)).subscribe(subscriber);
    ...
    @Test public void _01_When_Encrypted_Record_Has_Been_Persisted_And_Memory_Has_Been_Destroyed_Then_Retrieve_From_Disk() {
        ...
        TestSubscriber<Reply<List<Mock>>> subscriber = new TestSubscriber<>();
        providersRxCache.getMocksEncryptedWithDetailResponse(Observable.<List<Mock>>just(null)).subscribe(subscriber);
        subscriber.awaitTerminalEvent();
    ...
    @Test public void _02_If_Class_Has_Been_Annotated_With_EncryptedKey_Then_Only_Encrypt_When_Provider_Has_Been_Annotated_With_Encrypt() {
        initProviders();

        TestSubscriber<Reply<List<Mock>>> subscriber = new TestSubscriber<>();
        providersRxCache.getMocksNotEncryptedWithDetailResponse(Observable.<List<Mock>>just(null)).subscribe(subscriber);
        subscriber.awaitTerminalEvent();
    ...
    public interface ProvidersRxCache {
        ...
        @Encrypt
        Observable<Reply<List<Mock>>> getMocksEncryptedWithDetailResponse(Observable<List<Mock>> mocks);

        Observable<Reply<List<Mock>>> getMocksNotEncryptedWithDetailResponse(Observable<List<Mock>> mocks);
    }
}
