...
public class ProvidersRxCacheTest {
  ...
  @Test public void _00_Use_Expired_Data() {
    ...
    observer = new TestObserver<>();
    providersRxCache.getMocksListResponseOneSecond(Observable.just(Arrays.asList(new Mock())))
        .subscribe(observer);
    observer.awaitTerminalEvent();
  ...
  private Observable<Mock[]> createObservableMocksArray(int size) {
    return createObservableMocks(size)
        .map(new Function<List<Mock>, Mock[]>() {
          @Override
          public Mock[] apply(List<Mock> mockList) throws Exception {
