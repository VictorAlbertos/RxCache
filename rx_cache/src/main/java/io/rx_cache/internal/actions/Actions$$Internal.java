...
public class Actions$$Internal<T> {
    protected Observable<List<T>> cache;
    protected final Evict<T> evict;

    public Actions$$Internal(Evict<T> evict, Observable<List<T>> cache) {
        this.evict = evict;
        this.cache = cache;
    }
     ...
     * @return itself
     */
    public Actions$$Internal<T> add(Func2 func2, T element) {
     ...
     * @return itself
     */
    public Actions$$Internal<T> addFirst(T element) {
     ...
     * @return itself
     */
    public Actions$$Internal<T> addLast(T element) {
     ...
     * @return itself
     */
    public Actions$$Internal<T> addAll(final Func2 func2, final List<T> elements) {
    ...
    public Observable<List<T>> toObservable() {
        return cache.flatMap(new Func1<List<T>, Observable<List<T>>>() {
            @Override public Observable<List<T>> call(List<T> elements) {
                return evict.evict(elements);
            }
        });
    }
    ...
    interface Evict<T> {
        Observable<List<T>> evict(final List<T> elements);
    }
